...
{
    private static List<string> ConvertBlocks(
        ...
        int quoteDepth)
    {
        ...
        for (var index = 0; index < nodes.Count; index++)
        {
            ...
            if (context.DialectAdapter.TryConvertListLikeBlock(node, quoteDepth, out var customList))
            {
                blocks.Add(customList);
                continue;
            }

            if (IsStructuralContainer(node))
            {
                var nestedBlocks = ConvertBlocks(node.ChildNodes, context, inferredHeadingLevels, quoteDepth);
            ...
            {
                blocks.Add(ConvertList(node, context, inferredHeadingLevels, quoteDepth, 0));
                continue;
            }

            if (node.Name.Equals("table", StringComparison.OrdinalIgnoreCase))
            {
                if (TryConvertSingleCellTableAsQuote(node, context, inferredHeadingLevels, quoteDepth, out var quotedTable))
                {
            ...
            if (IsQuoteBlock(node, context.DialectAdapter))
            {
                var nested = ConvertBlocks(node.ChildNodes, context, inferredHeadingLevels, quoteDepth + 1);
                if (nested.Count > 0)
                {
            ...
            if (TryGetSemanticHeadingLevel(node, context.DialectAdapter, out var headingLevel) ||
                inferredHeadingLevels.TryGetValue(node, out headingLevel))
            {
            ...
            {
                blocks.AddRange(ConvertTextBlocks(ConvertInlines(node, context), quoteDepth, context, inferredHeadingLevels, node));
                continue;
            }

            if (TryConvertInlineSequence(nodes, ref index, context, inferredHeadingLevels, quoteDepth, out var inlineBlocks))
            {
        ...
        int indentLevel)
    {
        ...
        foreach (var child in listNode.ChildNodes)
        {
            ...
            if (child.Name.Equals("li", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    lines.Add(ConvertList(nested, context, inferredHeadingLevels, quoteDepth, indentLevel + 1));
                }
                ...
                foreach (var nestedBlock in nestedBlockContainers)
                {
                    ...
                    var nestedBlocks = ConvertBlocks(nestedBlock.ChildNodes, context, inferredHeadingLevels, 0);
            ...
            if (child.Name is "ul" or "ol")
            {
                ...
                var nestedList = ConvertList(child, context, inferredHeadingLevels, quoteDepth, nestedIndentLevel);
            ...
            {
                var nestedIndentLevel = hasRenderedListItem ? indentLevel + 1 : indentLevel;
                var nestedBlocks = ConvertBlocks(child.ChildNodes, context, inferredHeadingLevels, 0);
                foreach (var nestedLine in nestedBlocks.Where(line => !string.IsNullOrWhiteSpace(line)))
                {
        ...
        out string markdown)
    {
        ...
        var cell = cells[0];
        var blocks = ConvertBlocks(cell.ChildNodes, context, inferredHeadingLevels, quoteDepth + 1);
        if (blocks.Count == 0)
        {
            var fallback = ConvertTextBlocks(ConvertInlines(cell, context), quoteDepth + 1, context, inferredHeadingLevels, cell);
